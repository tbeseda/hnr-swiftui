...
Two settings. `classifyAIStories` runs the classifier: each stored story is classified as AI-topic or not, in the background, by keyword lists and Apple's on-device Foundation Models framework. `hideAIStories` is the "Hide AI stories" toggle in the toolbar filter panel, a view filter alongside the community-post and front-page filters, with "N of M stories are AI" spelled out beneath it. The panel shows the toggle whenever the model is available, and turning it on also turns classification on, so the one-click path works without a trip to Settings; Settings > Experimental > "Classify stories with Apple Intelligence" is the switch to turn classification back off. (An earlier Settings label, "AI story filter", read as if it were the filter itself.) (Earlier cuts -- a pressed `sparkles` toggle meaning "hidden", then a segmented "All N | No AI M" picker -- tested as counterintuitive and too wordy for the toolbar respectively.) The split matters: the user wants to hide AI bursts for a while and bring them back, so classification must keep running while hiding is off, otherwise a quick "hide" would find the newest stories unclassified. Requires Apple Intelligence to be enabled on an Apple silicon Mac; `StoryClassifier.unavailableReason` explains why the Settings toggle is disabled, and the panel omits the AI toggle entirely. No API keys. Two things do leave the Mac: a request to a story's own site for its page description (stories from the past week only, see below), and a fetch of the keyword lists from this repo on GitHub.

**Three stages, and the prompt shape is load-bearing.** `StoryClassifier` first checks the title and host against the keyword lists in `HNReader/RemoteConfig.json` (AI, LLM, GPT, OpenAI, Claude, "machine learning", openai.com, ...); a hit is `.ai` with no model call, which settles ~25% of the store instantly. The rest go to the model with a strict "default is no" prompt, title and hostname only. Stories under a week old that the model didn't call AI get a third pass: the app fetches the story URL's `<head>` and runs the same keyword lists over its `og:description`, `twitter:description`, `description`, and `og:title` tags. Measured Sept 2026 on the "26.4" model against 82 hard negatives (generic titles a permissive prompt had flagged: "bzip3", "Audacity 4.0", "Haiku R1" the OS) and 44 clear positives: the permissive rubric flagged 69/82 negatives; the strict prompt alone flagged 0/82 but missed 15/44 positives, mostly titles that literally say "AI"; keywords catch 37/44 with 0/82 false positives. Measured 21 Sept 2026 on the 80 newest stories the two title stages had passed as not AI: 75 pages answered (four 403s, one 401), 57 had a description tag, fetch p50 0.39 s and p90 1.0 s, and the head ended within 6 KB at the median and 58 KB at p90. The keyword lists over the description tags flipped 8 stories, 6 of them AI ("Jev-Leftpad", whose description reads "Left-pad strings with TypeSafe AI's Jev"; "Fable 5 – Median thinking declined in August"), 1 borderline, 1 wrong (a Cloudflare post whose description mentions "AI orchestration libraries"). The strict prompt shown the same excerpts said yes to 2 of 70, saying no even to "After Anthropic made Fable 5 permanently available", so the model never sees page text: only the keyword pass does. Body text and HN comments were measured and rejected: 2 of 6 paragraph hits and about two thirds of comment hits were passing mentions. Keep the keyword lists conservative -- Cursor, bare Grok, Haiku, Astra, agent, and model are all common words elsewhere; anchor product names on a version number (`Fable \d`, `Grok \d`, `GLM-?\d`) where the bare word is common. Prefer fewer false positives: a hidden story is a silent loss, a shown AI story is a visible nuisance.

**Keyword matching.** The lists are ICU regex fragments, compiled once per load into `NSRegularExpression` (`AIKeywords`), which is immutable and `Sendable`, so the compiled lists live in the classifier struct; Swift `Regex` is neither `Sendable` nor cheap to compile (~0.5 ms, against 1 µs per match). `terms` are wrapped in `\b...\b` and matched case-sensitively so AI doesn't hit "aim" or lowercase prose; `phrases` and `hosts` match anywhere, case-insensitively. ICU's `\b` is the simple `\w`/`\W` kind and splits "Claude's" and "5.1"; Swift `Regex`'s default Unicode word boundaries treat each as one word, which is why the old literal regexes missed every possessive.

**The page fetch is scoped on purpose.** It costs a request to the story's host from the user's IP for a story they may never click. So it is limited to stories under seven days old at classification time, runs only while classification is on, uses its own ephemeral `URLSession` (two connections per host, 6 s timeout, no cookies, `Accept: text/html`) with the User-Agent `HNReader/<version> (macOS; +https://github.com/tbeseda/hnr-swiftui)`, and reads the response only until `</head>` or 256 KB. It runs concurrently with the model call (`async let`) and is cancelled unawaited when the model says yes, so the pass stays model-bound. Non-HTML content types, non-200 responses, and fetch errors leave the model's verdict in place; nothing is retried.

**Remote config.** `HNReader/RemoteConfig.json` is both bundled into the app (the built-in lists, and the defaults until a fetch lands) and fetched from `https://raw.githubusercontent.com/tbeseda/hnr-swiftui/main/HNReader/RemoteConfig.json` at every refresh and whenever classification is turned on, while classification is on. It is namespaced by top-level key -- `aiKeywords` today -- and `schemaVersion` moves only for a change an older app can't read; unknown keys are ignored, so a new namespace is additive. A fetch failure (network, non-200, malformed JSON, unsupported schema, a fragment that doesn't compile) keeps the lists already in use and shows a notice under the toolbar that names GitHub and the reason, repeated in the Settings footer. A successful load re-runs the keyword stage over every stored story not already AI (`AppState.reapplyKeywords`, microseconds per story), so a term added to the list applies to stored titles at once; it reaches page descriptions only for stories classified after the change, since descriptions aren't stored. Removing a term never un-flags a story; bump `promptVersion` for that. Until the file exists on `main`, the fetch is a 404 and the notice shows.
...
**Where it runs.** `classifyPendingStories()` is called after each refresh, after each background check, and when `classifyAIStories` turns on -- only while that setting is on, regardless of whether hiding is on. The background check additionally classifies its few new finds inline (`classifyNow`) before recounting, so the new-story badge excludes AI stories while hiding instead of promising stories that refresh then hides. It processes stored stories lacking a current verdict, newest first, one at a time, in a single `Task`; a second call while a pass is running is a no-op and the running pass picks up newly stored stories and a keyword list that loads mid-pass. The page fetch happens inside `classify`, so it overlaps the model call rather than adding to it; a first pass over a full 14-day store (~1000 stories) still takes about five minutes.

**Persistence and versioning.** Verdicts live in `~/Library/Application Support/HNReader/verdicts.json` as `[storyID: StoredVerdict]`, pruned with the store. Each records the stage that produced it (`source`: title, model, or page; shown in the expanded row) and is pinned to `StoryClassifier.promptVersion` (bump it when the instructions, prompt, stages, or built-in lists change) and `StoryClassifier.modelVersion` (Apple ships distinct models at 26.0, 26.4, 27.0). Stale verdicts still filter the display until recomputed, so a version bump doesn't flash unfiltered stories.

**Known gaps.** Stories first discovered by a refresh fan-out show once unfiltered until the next refresh or the next time hiding is turned on. A story already over a week old when first classified (the app was closed, or the store was reset) never gets the page stage. The page stage's known error sources are sites that refuse the fetch (about 5%: Reuters, some forums) and site-level boilerplate descriptions ("a blog about AI and ...").
